...
/*   Updated: 2017/10/05 23:52:06 by anonymous        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		NinjaTrap::NinjaShoeBox(SuperTrap & target){
	if (this->ImAlive()){
			std::cout << "FR4G-TP "<< this->_name << " attacks " << target.getName() << " with a casque Bose, causing "
...
NinjaTrap::~NinjaTrap( void){
	std::cout << this->_name << ": never say never" << std::endl;
	//this->~ClapTrap();
	return;
}
